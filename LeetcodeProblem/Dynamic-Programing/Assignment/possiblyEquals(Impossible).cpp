...
public:
    ...
    bool possiblyEquals(string &s1, string &s2,int i=0,int j=0,int diff=0) {
        ...
        if (!visited[i][j][1000+diff]) {
            ...
            if (i<s1.size()&&isdigit(s1[i])) return processDigits(s1,i,-1);
            if (j<s2.size()&&isdigit(s2[j])) return processDigits(s2,j,1);
            if (diff>0) return i<s1.size()&&possiblyEquals(s1,s2,i+1,j,diff-1);
            if (diff<0) return j<s2.size()&&possiblyEquals(s1,s2,i,j+1,diff+1);
